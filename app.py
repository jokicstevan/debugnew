        ...
        for v, route in enumerate(self.routes):
            ...
            if do_wages:
                work_mins = route_working_minutes(
                    route, depot, self.dist_mat, self.time_mat, self.tw, self.svc,
                    start, self.svc_map)
                total += (work_mins / 60.0) * DRIVER_WAGE_RSD_PER_HOUR
            if do_dist:
...
def optimize():
    try:
        ...
        for v_idx, route in enumerate(state.routes):
            ...
            wage_cost   = round(work_h * DRIVER_WAGE_RSD_PER_HOUR, 0)
        ...
        return jsonify({
            ...
            "driver_wage_rsd_h":   DRIVER_WAGE_RSD_PER_HOUR,
            "fuel_price_rsd_l":    FUEL_PRICE_RSD_PER_LITRE,
            "total_time_h":        hours,
            "total_time_m":        mins,
...
def generate_pdf():
    ...
    for vr in data.get("vehicle_routes",[]):
        ...
        if vr.get("stops"):
            stop_data = [["#","Customer","Arrival","Depart","Window","Pkg1","Pkg2","Pkg3","Vol m\u00b3"]]
            for i, s in enumerate(vr["stops"], 1):
                ...
                stop_data.append([str(i), s["name"][:20],
                                   ...
                                   str(pc[0]), str(pc[1]), str(pc[2]),
                                   f"{float(vol):.2f}"])
            st = Table(stop_data, colWidths=[18,95,50,48,68,22,22,22,34])
