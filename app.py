...
def _job_create(job_id: str, owner: str):
    ...
    if not DATABASE_URL:
        ...
        logger.info(f"Job {job_id} created (in-memory) by {owner}")
        return
    try:
        conn = _get_db_conn()
        cur = conn.cursor()
...
# --- Cancel job endpoint (uses login_required, now after auth) -----------------
...
# ─── ALNS operators (unchanged, but with cancellation support) ────────────────
...
def _shaw_remove(state, rng):
    ...
    for d in cust_data:
        ...
        tw_diff = abs(d["tw_center"] - cust_data[seed_idx]["tw_center"]) / 480.0
...
def generate_pdf():
    ...
    if map_b64:
        try:
            ...
            max_h = 280
            rl_img = RLImage(img_buf, width=available_w, height=max_h, kind='bound')
