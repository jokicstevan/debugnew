...
# Threshold: when the number of pairs to fetch exceeds this, switch from
# individual /v8/routes calls to the HERE Matrix API (/v8/matrix).
# The Matrix API handles all origin×destination combinations in a single
# batched HTTP call, making it far more efficient for large customer sets.
# Below this threshold, per-pair calls with a thread pool are just as fast
# and simpler to reason about.
_HERE_MATRIX_API_THRESHOLD = 40   # pairs; roughly N >= 8 locations

# HERE Matrix API (sync) supports up to 15 origins per call without OAuth2.
# We chunk unique origins into batches of this size.
_HERE_MATRIX_SYNC_ORIG_LIMIT = 15


def _here_matrix_api_batch(orig_indices, dest_indices, locations, dep_time):
    """Single synchronous call to the HERE Matrix API v8.

    Sends one batch with len(orig_indices) origins and len(dest_indices)
    destinations.  Returns a dict {(i, j): (dist_km, time_min)} for every
    successfully resolved origin/destination pair, or None on API failure.
    """
    orig_locs = [{"lat": locations[i]["lat"], "lng": locations[i]["lng"]}
                 for i in orig_indices]
    dest_locs = [{"lat": locations[j]["lat"], "lng": locations[j]["lng"]}
                 for j in dest_indices]

    payload = {
        "origins":          orig_locs,
        "destinations":     dest_locs,
        "regionDefinition": {"type": "autoCircle"},
        "routingMode":      "fast",
        "transportMode":    "car",
    }
    if dep_time:
        payload["departureTime"] = dep_time
    ...
    try:
        ...
        if resp.status_code != 200:
            print(f"[HERE matrix API] batch failed: {resp.status_code} {resp.text[:300]}")
            return None
        ...
        n_dests = len(dest_indices)

        results = {}
        for ci, orig_i in enumerate(orig_indices):
            for di, dest_j in enumerate(dest_indices):
                if orig_i == dest_j:
                    continue
                flat = ci * n_dests + di
                ec = error_codes[flat] if flat < len(error_codes) else 0
                if ec != 0:
                    continue          # leave as sentinel — caller handles
                dist_m = distances[flat]    if flat < len(distances)    else None
                time_s = travel_times[flat] if flat < len(travel_times) else None
                if dist_m is not None and time_s is not None:
                    results[(orig_i, dest_j)] = (dist_m / 1000.0, time_s / 60.0)
        return results

    except Exception as exc:
        print(f"[HERE matrix API] exception: {exc}")
        return None


def _fetch_here_matrix_large(locations, fetch_set, hav_km, sf, dep_time):
    """Use the HERE Matrix API for large pair sets.

    Unique origin indices from fetch_set are chunked into batches of at most
    _HERE_MATRIX_SYNC_ORIG_LIMIT.  All needed destination indices are sent
    with every batch.  Results are merged into the full N×N matrix.

    Returns (dist_mat, time_mat) or (None, None) on failure.
    """
    n = len(locations)
    dist_mat = [[0.0]*n for _ in range(n)]
    time_mat = [[0.0]*n for _ in range(n)]

    # Sentinel pre-fill for pairs NOT in fetch_set
    if hav_km is not None:
        for i in range(n):
            for j in range(n):
                if i != j and (i, j) not in fetch_set:
                    sentinel_d = float(hav_km[i][j]) * sf
                    dist_mat[i][j] = sentinel_d
                    time_mat[i][j] = sentinel_d / 30.0 * 60.0

    orig_set = sorted({i for (i, j) in fetch_set})
    dest_set = sorted({j for (i, j) in fetch_set})

    total_calls    = 0
    total_pairs    = 0
    sentinel_extra = 0

    for chunk_start in range(0, len(orig_set), _HERE_MATRIX_SYNC_ORIG_LIMIT):
        chunk        = orig_set[chunk_start:chunk_start + _HERE_MATRIX_SYNC_ORIG_LIMIT]
        batch_result = _here_matrix_api_batch(chunk, dest_set, locations, dep_time)
        if batch_result is None:
            print("[HERE matrix API] batch returned None — aborting Matrix API path")
            return None, None
        total_calls += 1

        for (orig_i, dest_j), (dist_km, time_min) in batch_result.items():
            if (orig_i, dest_j) in fetch_set:
                dist_mat[orig_i][dest_j] = dist_km
                time_mat[orig_i][dest_j] = time_min
                total_pairs += 1

        # Pairs the Matrix API couldn't resolve → haversine sentinel
        if hav_km is not None:
            for orig_i in chunk:
                for dest_j in dest_set:
                    if orig_i == dest_j:
                        continue
                    if (orig_i, dest_j) not in fetch_set:
                        continue
                    if dist_mat[orig_i][dest_j] == 0.0:
                        sentinel_d = float(hav_km[orig_i][dest_j]) * sf
                        dist_mat[orig_i][dest_j] = sentinel_d
                        time_mat[orig_i][dest_j] = sentinel_d / 30.0 * 60.0
                        sentinel_extra += 1

    print(f"[HERE matrix API] ✅ {n}×{n} matrix via Matrix API "
          f"({total_calls} batch calls, {total_pairs} pairs resolved, "
          f"{sentinel_extra} extra sentinel-filled)")
    return dist_mat, time_mat


def fetch_here_matrix(locations, pairs=None, hav_km=None, sentinel_factor=None,
                      departure_min=None):
    """Build N×N matrix using HERE live-traffic data.

    Strategy
    --------
    Small pair sets  (≤ _HERE_MATRIX_API_THRESHOLD pairs):
        Individual /v8/routes calls in a thread pool.

    Large pair sets  (> _HERE_MATRIX_API_THRESHOLD pairs):
        HERE Matrix API (/v8/matrix) — one batched POST per chunk of
        _HERE_MATRIX_SYNC_ORIG_LIMIT origins.  Drastically fewer HTTP
        round-trips for 30+ customer problems.
        Falls back to per-pair calls if the Matrix API fails.

    Parameters
    ----------
    locations        : list of {"lat": float, "lng": float}
    pairs            : optional set of (i, j) tuples to fetch from HERE.
                       Absent pairs get haversine-sentinel values so the
                       VRP solver never routes through them.
    hav_km           : optional np.ndarray of haversine distances (km).
    sentinel_factor  : haversine multiplier for sentinel values.
    departure_min    : minutes since midnight for planned departure time.
    """
    if not HERE_API_KEY:
        return None, None
    ...
    # ── Large sets: HERE Matrix API (batched) ─────────────────────────────────
    if len(fetch_set) > _HERE_MATRIX_API_THRESHOLD:
        print(f"[HERE matrix] {len(fetch_set)} pairs → using HERE Matrix API "
              f"(threshold={_HERE_MATRIX_API_THRESHOLD})")
        dm, tm = _fetch_here_matrix_large(locations, fetch_set, hav_km, sf, dep_time)
        if dm is not None:
            return dm, tm
        print("[HERE matrix] Matrix API failed — retrying with per-pair calls")

    # ── Small sets: concurrent per-pair /v8/routes calls ─────────────────────
    dist_mat = [[0.0]*n for _ in range(n)]
    time_mat = [[0.0]*n for _ in range(n)]
    ...
    print(f"[HERE matrix] ✅ {n}×{n} matrix built with per-pair calls "
