...
_job_cancel_events: dict = {}  # job_id → threading.Event (set to request cancellation)


class JobCancelledError(Exception):
    """Raised inside the optimisation thread when a cancel has been requested."""


def _job_create(job_id: str, owner: str):
    _job_cancel_events[job_id] = threading.Event()
    print(f"[jobs] ▶ created job {job_id} for owner={owner}")
    if not DATABASE_URL:
...
def _job_set_cancelled(job_id: str):
    """Mark a job as cancelled in the store."""
    print(f"[jobs] ✖ job {job_id} cancelled")
    if not DATABASE_URL:
        if job_id in _local_jobs:
            _local_jobs[job_id].update({"status": "cancelled", "error": "Cancelled by user"})
        return
    try:
        conn = _get_db_conn()
        cur  = conn.cursor()
        cur.execute(
            "UPDATE grps_jobs SET status='cancelled', error='Cancelled by user', updated_at=NOW() WHERE job_id=%s",
            (job_id,)
        )
        conn.commit()
        conn.close()
    except Exception as exc:
        print(f"[jobs] set_cancelled failed: {exc}")
        _local_jobs[job_id] = {"status": "cancelled", "error": "Cancelled by user"}


def _job_is_cancelled(job_id: str) -> bool:
    """Return True if the cancel event for this job has been set."""
...
def _or_opt_remove(state, rng):
    """Or-opt destroy: remove a contiguous chain of 1, 2, or 3 stops from a route.

    Selects the chain whose removal most improves (or least worsens) the route
    distance, biased by random selection so the operator explores widely.
    This is a targeted destroy that keeps clusters intact while freeing
    expensive-to-reach outlier chains for resertion elsewhere.
    """
    s = state.copy()
    all_routes = [(v, r) for v, r in enumerate(s.routes) if len(r) >= 1]
    if not all_routes:
        return s

    chain_len = int(rng.choice([1, 2, 3]))
    candidates = []
    for v, route in all_routes:
        if len(route) < chain_len:
            continue
        d  = s.depot_of[v]
        dm = s.dist_mat
        for start in range(len(route) - chain_len + 1):
            chain = route[start:start + chain_len]
            prev  = route[start - 1] if start > 0 else d
            nxt   = route[start + chain_len] if (start + chain_len) < len(route) else d
            # Cost saving from removing this chain
            old_cost = dm[prev][chain[0]] + dm[chain[-1]][nxt]
            new_cost = dm[prev][nxt]
            saving   = old_cost - new_cost
            # Jitter by a random perturbation to avoid always picking the same chain
            jitter = float(rng.random()) * 0.05 * max(abs(saving), 1.0)
            candidates.append((saving + jitter, v, start, chain_len))

    if not candidates:
        return s

    # Pick probabilistically among top candidates (softmax-style)
    candidates.sort(reverse=True)
    top_k = max(1, len(candidates) // 3)
    chosen = candidates[int(rng.integers(0, top_k))]
    _, v, start, clen = chosen
    del s.routes[v][start:start + clen]
    print(f"[ALNS] or_opt_remove: v={v} chain_len={clen} start={start}")
    return s
...
def _alns_optimize(fleet, dist_mat, time_mat, n_depots, n_cust, tw, demands,
                   ...
                   no_wait=False, cancel_event=None, job_id=None):
    """Run a single ALNS optimisation with the given fleet (list of vehicle dicts).

    Improvements over baseline:
    ─ 6 destroy operators: random, worst, TW, capacity, overlap, or-opt
    ─ 2 repair operators: greedy insert, regret insert
    ─ Adaptive weight learning (ALNS-style σ scores, reaction factor r=0.15):
        σ1=3 new global best  σ2=2 improved current  σ3=1 SA-accepted  σ4=0 rejected
    ─ Cancellation support: checks cancel_event every iteration
    ─ Heavy logging: iteration milestones, operator selection, temperature decay
    """
    num_v = len(fleet)
    all_ci = list(range(n_depots, n_depots + n_cust))

    t_start = time.time()
    print(f"[ALNS{'|'+job_id[:8] if job_id else ''}] start: {n_cust} customers, "
          f"{num_v} vehicles, max_iter={max_iter}, T0={temperature:.1f}, "
          f"obj_weights={obj_weights}")
    ...
    print(f"[ALNS] initial objective={best_obj:.2f} (inf={best_obj==float('inf')}), "
          f"active_vehicles={sum(1 for r in state.routes if r)}/{num_v}")

    # ── 6 destroy + 2 repair operators ─────────────────────────────────────────
    destroy = [_rand_remove, _worst_remove, _tw_remove, _cap_remove,
               _overlap_remove, _or_opt_remove]
    repair  = [_greedy_insert, _regret_insert]
    destroy_names = ["rand", "worst", "tw", "cap", "overlap", "or_opt"]
    repair_names  = ["greedy", "regret"]

    # Adaptive operator weights (ALNS σ-scoring, reaction factor r)
    REACT   = 0.15   # how fast weights adapt (0 = no adapt, 1 = replace)
    SIGMA1  = 3.0    # new global best found
    SIGMA2  = 2.0    # improved current (but not global best)
    SIGMA3  = 1.0    # SA-accepted but worse
    # σ4 = 0 (rejected) — no update needed

    dw = [1.0] * len(destroy)   # operator selection weights
    rw = [1.0] * len(repair)
    d_uses   = [0] * len(destroy)    # usage counts (for logging)
    r_uses   = [0] * len(repair)
    d_scores = [0.0] * len(destroy)  # accumulated sigma scores
    r_scores = [0.0] * len(repair)
    ...
    for it in range(max_iter):
        ...
        if cancel_event is not None and cancel_event.is_set():
            print(f"[ALNS] ✖ cancellation requested at iteration {it}/{max_iter} "
        ...
        # SA accept / reject and σ scoring
        sigma = 0.0
        if delta < 0:
            accepted = True
            if cand_obj < best_obj:
                best, best_obj = cand.copy(), cand_obj
                sigma = SIGMA1
            else:
                sigma = SIGMA2
        elif float(rng.random()) < math.exp(-delta / max(temp, 1e-9)):
            accepted = True
            sigma = SIGMA3
        else:
            accepted = False
        ...
        # Adaptive weight update (weighted moving average)
        d_uses[di] += 1
        r_uses[ri] += 1
        if sigma > 0:
            d_scores[di] += sigma
            r_scores[ri] += sigma
            dw[di] = max(0.1, (1.0 - REACT) * dw[di] + REACT * sigma)
            rw[ri] = max(0.1, (1.0 - REACT) * rw[ri] + REACT * sigma)
    ...
    # Final operator usage report
    print(f"[ALNS] ✅ done in {elapsed:.1f}s  best_obj={best_obj:.2f}  "
          f"active_vehicles={sum(1 for r in best.routes if r)}/{num_v}")
    for i, name in enumerate(destroy_names):
        if d_uses[i]:
            avg_score = d_scores[i] / d_uses[i]
            print(f"[ALNS]   destroy '{name}': used={d_uses[i]:4d}  "
                  f"avg_σ={avg_score:.3f}  final_w={dw[i]:.3f}")
    for i, name in enumerate(repair_names):
        if r_uses[i]:
            avg_score = r_scores[i] / r_uses[i]
            print(f"[ALNS]   repair  '{name}': used={r_uses[i]:4d}  "
                  f"avg_σ={avg_score:.3f}  final_w={rw[i]:.3f}")
...
def optimize():
    ...
    until status == 'done', 'error', or 'cancelled'.
    ...
    cancel_event = _job_cancel_events[job_id]

    def _run():
        try:
            result = _do_optimize(data, user, job_id=job_id, cancel_event=cancel_event)
            _job_set_done(job_id, result)
        except JobCancelledError as exc:
            print(f"[optimize job {job_id}] cancelled: {exc}")
            _job_set_cancelled(job_id)
        except Exception as exc:
            import traceback
...
def optimize_status(job_id):
    """Poll endpoint — reads job state from Postgres, works on any worker.
      ...
      { ok:false, status:'cancelled' }             — cancelled by user
    ...
    if job["status"] == "cancelled":
        return jsonify({"ok": False, "status": "cancelled", "error": "Job was cancelled"})
...
@app.route("/api/optimize/<job_id>/cancel", methods=["POST"])
@login_required
def cancel_optimize(job_id):
    """Signal a running optimisation job to stop.
    Sets a threading.Event that the solver loop checks every iteration.
    The job is marked 'cancelled' in the DB; the poll endpoint returns it.
    """
    ev = _job_cancel_events.get(job_id)
    if ev is None:
        return jsonify({"ok": False, "error": "Job not found or already finished"}), 404
    ev.set()
    print(f"[jobs] ✖ cancel requested for job {job_id} by {session.get('user', '?')}")
    return jsonify({"ok": True, "message": "Cancellation requested"})


def _do_optimize(data, user, job_id=None, cancel_event=None):
    ...
    Raises JobCancelledError if cancel_event is set during execution.
    """
    _t0_total = time.time()
    _jid = job_id[:8] if job_id else "local"
    log = lambda msg: print(f"[job:{_jid}] {msg}")

    log(f"▶ _do_optimize started by user={user}")
    ...
    log(f"Problem size: {n_depots} depots, {n_cust} customers, {len(fleet_cfg)} vehicle type(s), "
        f"algorithm={algorithm}, max_iter={max_iter}, use_tw={use_tw}")
    ...
    log(f"Phase 1: building distance/time matrix for {len(all_locs_orig)} locations "
        f"(k_nearest={k_nearest}, sentinel={sentinel_factor}, departure={departure_min})")
    _t1 = time.time()
    ...
    # Cancellation checkpoint — matrix fetch is the longest pre-solver step
    if cancel_event is not None and cancel_event.is_set():
    ...
    if "Nearest Neighbor" in algorithm:
        state = optimize_nn(dist_mat, time_mat, n_depots, n_cust, tw, demands,
                            ...
                            use_volume_cap=use_volume_cap, use_weight_cap=use_weight_cap,
                            fuel_price_rsd_l=fuel_price_rsd_l, driver_wage_rsd_h=driver_wage_rsd_h,
                            fuel_load_factor=fuel_load_factor, **{k: v for k, v in adv_kwargs.items()
                                                                   if k not in ('cancel_event', 'job_id')})
    ...
    log(f"✅ complete in {time.time()-_t0_total:.2f}s total — "
        f"{len(vehicle_routes)} vehicle routes, "
...
def export_workspace(ws_id):
    """Return a workspace as a downloadable JSON file.
    Includes depots, customers, fleet, settings, and (if present) the last
    optimisation result — everything needed to reproduce a run.
    """
    if not DATABASE_URL:
        return jsonify({"ok": False, "error": "Database not configured"}), 503
    try:
        ...
        export_data = {
            "grps_export_version": "1.0",
            "exported_at":         datetime.utcnow().isoformat() + "Z",
            "exported_by":         session.get("user", "unknown"),
            "workspace_id":        row[0],
            "name":                row[1],
            "description":         row[2] or "",
            "created_by":          row[3],
        ...
        safe_name = "".join(c if c.isalnum() or c in "-_ " else "_" for c in (row[1] or "workspace"))
        fname = f"grps_workspace_{safe_name}_{datetime.utcnow().strftime('%Y%m%d_%H%M%S')}.json"
        buf = io.BytesIO(json.dumps(export_data, indent=2, ensure_ascii=False).encode("utf-8"))
        buf.seek(0)
        print(f"[workspaces] export ws_id={ws_id} name={row[1]!r} by={session.get('user','?')}")
